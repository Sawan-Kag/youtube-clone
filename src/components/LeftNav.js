...
const LeftNav = () => {
  const { selectedCategory, setSelectedCategory, mobileMenu, theme, setTheme } = useContext(DataContext);
  ...
  return (
    <div
      ...
      <div className="flex px-5 flex-col">
          ...
          return (
            <div key={item.name}>
              <LeftNavMenuItem
                ...
                className={`${
                  selectedCategory === item.name ? "bg-white/[0.15] md:bg-black/[0.15] dark:bg-white/[0.15]" : ""
                }`}
              />
              {item.divider && (
        ...
        <hr className="my-5 border-white/[0.2] md:border-black/[0.2] border dark:border dark:border-white/[0.2]" />
        <div className="text-white/[0.5] md:text-black/[0.7] dark:text-white/[0.5] text-[12px]">Build by Sawan Kag</div>
      </div>
    </div>
  );
};
